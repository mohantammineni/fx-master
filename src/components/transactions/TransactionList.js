  ...
  return (
    <>
      {transactionList.map((item, index) => {
        ...
        const senderName = item.type == 'debit' ? JSON.parse(item.meta).second_beneficiary_name : JSON.parse(item.meta).sender_name;
        ...
        return (
          <div key={index} className="rounded-3xl bg-white pb-4">
            <div className="flex justify-between items-center px-8 py-4">
              ...
              <div className="text-base font-semibold text-black">{item.settled_currency} {numberWithCommas(item.settled_amount)}</div>
            </div>
          </div>
        )
      }
