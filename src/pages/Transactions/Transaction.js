...
function Transaction() {
  ...
  async function loadTransdetails(ref, amount, paymentDate, beneName, beneAccount, card_fees, status) {
  ...
  const navigatetotransaction = async (paymentMethod, metaDetails, date, send) => {
    ...
    paymentMethod == "manual_transfer" && JSON.parse(metaDetails).city != "CB" ?
      Object.prototype.hasOwnProperty.call(JSON.parse(metaDetails), 'currency_cloud_payment_id') ?
        ...
        loadTransdetails(JSON.parse(metaDetails).reference_no, JSON.parse(metaDetails).exchange_currency + " " + JSON.parse(metaDetails).recipient_amount, date, JSON.parse(metaDetails).second_beneficiary_name, JSON.parse(metaDetails).second_beneficiary_bank_account_number, JSON.parse(metaDetails).card_fees,JSON.parse(metaDetails).status) :
