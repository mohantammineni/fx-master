...
function Beneficiary({ beneficiaryList, sendDataToParent, pay = false }) {
  ...
  return (
    <div className="my-2">
      <div className="relative overflow-x-auto bg-white" style={{boxShadow: '0px 4px 4px rgba(0, 0, 0, 0.25)', borderRadius:'24px'}}>
  <table className="w-full text-sm text-left text-[#262424] font-bold bg-white border-collapse">
    ...
    {beneficiaryList?.length === 0 ? (
        ...
        beneficiaryList?.map((beneficiary) =>
         (
            <tr
              key={beneficiary.id}
              className="cursor-pointer hover:bg-gray-100 transition border-b border-[#F0F0F0]"
              onClick={(e) => {
            ...
            >
              ...
              <td className="flex px-6 py-4 text-left items-center">
                <div className="mx-2">
                  {Constants.CURRENCYID[beneficiary.country_id] &&
  ...
  className="flex items-center bg-[#029328] text-white px-4 py-1 rounded-lg shadow-md hover:bg-[#027a24] transition"
>
  <img src="/arrow-up.png" alt="Arrow" className="w-2 h-2 mr-1" />
  Pay
</button>
