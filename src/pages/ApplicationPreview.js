...
export default function ApplicationPreview() {
    ...
    const pin = Object.prototype.hasOwnProperty.call(paramsdata, 'pin') ? paramsdata.pin : null;
    ...
    const loginHandler = async () => {
        ...
        if(pin==null || pin=="")
        {
          enterpin = sessionStorage.getItem('enterPin');
        }

        const email = sessionStorage.getItem('user_email')
        await axios.post(Constants.BASE_URL + "API-FX-120-Login", {
            "email": email,
            "password": pin==null || pin=="" ? enterpin : pin
        }, {
            headers: {
