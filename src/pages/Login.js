  ...
  const handleLogin = async (e) => {
    ...
    await axios.post(Constants.BASE_URL + 'API-FX-120-Login', {
      ...
      if (resp.data.data.registration_step == 'account_preview') {
        ...
        setAsyncData("re_kyc", JSON.stringify(resp.data.need_rekyc));
        if(resp.data.need_rekyc==0)
        navigate('/')
        else
  ...
  return (
    <div className="flex h-screen">
      ...
      {Constants.MAINTENANCE ?
        <div className="w-1/2 flex flex-col justify-center items-start p-8 border-l"><h1>Currently we are under maintenance. Will be back soon. Sorry for the inconvenience.</h1></div>
        :
